...
public class Graph
{
	...
	public static double shortestPath(Vertex v1, Vertex v2)
	{
	...
	public static void makeShortestPath(Vertex v1)
	{
		Queue<Vertex> toBeChecked = new LinkedList<Vertex>();
		Vertex temp;
		...
		for(;toBeChecked.isEmpty() == false;)
		{
			...
			for(int count = 0; count<temp.edges.size(); count++)
			{
				...
				Vertex newVertex = temp.edges.get(count).second == temp ? temp.edges.get(count).first : temp.edges.get(count).second;
